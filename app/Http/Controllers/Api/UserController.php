...
class UserController extends Controller
{
   ...
   /**
     ...
     *             @OA\Property(property="deactivation_reason", type="string", example="No longer using the service")  
     *         )
     *     ),
     *     @OA\Response(
     *         response=200,
    ...
    public function deactivate(Request $request)
    {
        $request->validate([
            ...
            'deactivation_reason' => 'string|max:255', 
        ]);
        $user = User::findOrFail($request->user_id);
        $user->update([
            'is_active'       => false,
            'deactivated_by'  => Auth::id(),
            'deactivation_reason' => $request->deactivation_reason ?? null, // You can customize this
            'deactivated_at'  => now(), // Remove this if you don't have this column
        ]);

        // Revoke all API tokens of the deactivated user
        $user->tokens()->delete();
