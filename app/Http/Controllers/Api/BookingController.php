...
class BookingController extends Controller
{
     ...
     *     summary="Update booking(s) from cart",
     *     security={{ "bearerAuth": {} }},
     *     tags={"Booking"},
     *     @OA\Parameter(
     *         name="id",
    ...
    public function updateBooking(Request $request, $bookingId)
    {
        ...
        // Get parent booking
        $parentBooking = Booking::with('childBookings')
            ->where('id', $bookingId)
            ->where('user_id', $user->id)
        ...
        try {
            ...
            */
            ...
            $hoursPerService = $itemsCount > 0
                ? $totalHours / $itemsCount
                : $totalHours;
            ...
            */
            ...
            /*
            |--------------------------------------------------------------------------
            | Delete Existing Child Bookings
            |--------------------------------------------------------------------------
            |
            | We recreate the slot bookings based on the updated cart.
            |
            */

            $oldChildBookings = $parentBooking->childBookings()->get();

            foreach ($oldChildBookings as $oldChild) {
                $oldChild->items()->delete();
                $oldChild->delete();
            }

            /*
            |--------------------------------------------------------------------------
            | Create Updated Child Slot Bookings
            |--------------------------------------------------------------------------
            */
            ...
            foreach ($timeSlots as $index => $slot) {
                ...
                $slotHoursPerService = $itemsCount > 0
                ...
                */
                ...
                foreach ($cart->serviceItems as $item) {
                    if (
                        ...
                        array_key_exists($item->service_id, $request->service_time)
                    ) {
                        $pricePerMinute = $item->price / 60;

                        $finalPrice = $pricePerMinute *
                            $request->service_time[$item->service_id];
                ...
                */
                ...
                $child = Booking::create([
                    ...
                    'service_requirements' =>
                        $cart->service_requirements ?? null,
            ...
            */
            ...
            $parentBooking->serviceItems()->delete();
            ...
            */
            ...
            try {
                ...
                foreach ($childBookings as $childBooking) {
                    ...
                    Mail::send(
                        new BookingCreatedMail($childBooking)
                    );

                    // Socket event
                    event(
                        new NewBookingCreated($childBooking->id)
                    );
                }
                ...
                Log::error(
                    'Error sending booking update notifications: ' .
                    $e->getMessage()
                );
            }
            ...
            */
            ...
            return response()->json([
                ...
                'message' => count($childBookings) > 1
                    ? 'Recurring booking updated with ' .
                        count($childBookings) .
                        ' slots'
                ...
                'slot_bookings' =>
                    collect($childBookings)->map->load('items'),
            ]);
